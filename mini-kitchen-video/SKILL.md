...
4. After confirmation, run `node {baseDir}/scripts/video_gen.js wait --json '…'` (text-to-video, no `image`); match fields; parse stdout.
...
- Image URL must be public HTTPS; local paths fail—use an image host (e.g. imgbb, Cloudinary).
