---
name: shonen-battle-anime-video-gen
version: 1.0.0
description: "Create high-energy shonen-style battle clips—speed lines, battle aura, explosive VFX, dramatic poses, and punchy action pacing for short vertical anime. Use when you need shonen anime combat tension, dynamic camera, or image-to-video from a still hero or villain key art; expand briefs before submit. Paid API; default **KLING_V3_0_PRO** for layered combat; **VIDU_Q3** for long anime takes; **SEEDANCE_2_0** for cheap motion drafts."

tags: [anime, shonen, battle, action, speed-lines, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Shonen battle anime vertical video

**Shonen battle anime** in one line: **high contrast**, **saturated heroic color**, **speed lines**, **impact VFX**, **exaggerated motion**, **burning energy**—vertical hype cuts for feeds.

**Style anchor:** `shonen anime, dynamic action, speed lines, high energy, dramatic pose, battle aura`.

## Use when

- **Shonen** fight beats, **clash**, **power-up**, **finisher pose**, **battle aura** glow.
- Text or **single image-to-video** from character key art / splash.

**Not this skill:** slow **shojo romance** pastel default, **slice-of-life** calm dailies—unless the user asks for a deliberate hybrid.

**Models:** tiered—**Kling 3.0** (default best), **Vidu Q3** (good), **Seedance 2.0** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Seedance 2.0**: set `resolution` (`720p` default); **no** `negative_prompt`. **Vidu Q3**: pick `resolution` from frozen row.
...
- **Look / materials / VFX (genre):**
  - **Palette:** high contrast, punchy saturation, anime cel read, rim light on silhouettes, sparks and debris reads.
  - **VFX:** speed lines, shockwaves, energy bursts, ground cracks, impact frames (describe timing, not trademark shows).
  - **Character:** clear silhouette, exaggerated anticipation → release, readable fist/weapon arc.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** low-angle hero shots, fast push-in on impact, whip-pan between fighters, brief freeze-frame emphasis on peak pose.
  - **Framing:** **9:16** safe—keep faces and impact zone in upper-middle; avoid cropping critical motion at edges.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `live-action, photoreal skin, muddy motion blur, watermark, subtitles, low fps`.
...
- `9:16` shonen clash—two swordfighters, speed lines, low angle, battle aura, epic drums + impact SFX (generic)
- Power-up montage beat—energy spiral, freeze on scream pose, saturated anime cel
- Image-to-video: static hero splash → wind + aura ripple + camera push
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `KLING_V3_0_PRO` | 10 | Maximum **multi-hit choreography**, **negatives** for live-action bleed. |
| 👍 Good | `VIDU_Q3` | 10 | **Vidu Q3**—strong **anime-style** motion in **5 / 10 / 15s** with full **resolution** control. |
| ⚡ Fast | `SEEDANCE_2_0` | 10 | **Seedance 2.0**—fast **organic** timing sweeps; **`720p`** drafts. |
...
| Field | Value |
|-------|--------|
| `model` | `KLING_V3_0_PRO` (unless user picks another **listed** tier) |
...
> Skill: `shonen-battle-anime-video-gen`.
