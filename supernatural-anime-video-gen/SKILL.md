...
# Supernatural / paranormal anime vertical video

**Supernatural anime** in one line: **still world**, **one wrong light**, **paper talisman** flutter, **breath fog** odd.

**Style anchor:** `supernatural anime, spiritual energy, mystical symbols, eerie glow, paranormal mood`.

## Use when

- **Shrine** gate dusk, **spirit orb** **generic**, **barrier** ripple air.
- Text or **single image-to-video**.

**Not this skill:** **hard sci-fi** cockpit tech read as default.

**Models:** tiered—**Kling 2.6** (default best), **Seedance 2.0** (good), **Wan 2.6** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 2.6** (image): **omit** `aspect_ratio` and `resolution` unless API updates. **Seedance 2.0**: set `resolution` (`720p` default); **no** `negative_prompt`. **Wan 2.5 / 2.6**: **no** audio; **image** rows often **omit** `aspect_ratio`—see frozen table.
...
- **Look / materials / VFX (genre):**
  - **Glow:** teal/violet spirit light **anime**, not photoreal ghost photo.
  - **Symbols:** abstract talisman marks, rope **shimenawa** **generic** stylization.
  - **Mood:** quiet uncanny; respect cultural cues **without** copying specific franchise creatures.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** drift follow, slow orbit around floating anomaly, push through barrier shimmer.
  - **Framing:** **9:16**—vertical torii **generic** or narrow forest path.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `sci-fi HUD overload, bright idol stage, watermark`.
...
- `9:16` shrine dusk—barrier ripple, teal glow orb, slow orbit, wind chime + low hum
- Forest path—paper talisman flutter, fog crawl
- Image-to-video: spirit girl art → hair float + glow pulse
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `KLING_V2_6_PRO` | 10 | **Kling 2.6**—**teal** **glow** discipline. |
| 👍 Good | `SEEDANCE_2_0` | 10 | **Seedance 2.0**—**longer** **drift** shots. |
| ⚡ Fast | `WAN_2_6` | 10 | **Wan 2.6**—**negative_prompt** for **clean** **curse** motifs; **no audio**. |
...
> Skill: `supernatural-anime-video-gen`.
