---
name: horror-anime-video-gen
version: 1.0.0
description: "Unsettling horror anime atmosphere—negative space, sudden motion, haunted interiors, and psychological dread for vertical scares. Use when you need eerie corridors, ghost implications, or dread motion from spooky art; expand briefs before submit. Paid API; **SORA_2_PRO** default **uncanny** **silent** **8s**; **KLING_V3_0_PRO** when **SFX** + **anime** dread needed; **PIKA 2.2** **stylized** **jumps** (omit **aspect_ratio**)."

tags: [anime, horror, psychological, haunted, dread, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Horror anime vertical video

**Horror anime** in one line: **too-quiet hallway**, **wrong shadow**, **frame skip** unease, **breath** close.

**Style anchor:** `horror anime, eerie silence, unsettling shadows, haunted atmosphere, psychological fear`.

## Use when

- **Flicker** light, **door** creak beat, **mirror** doubt **generic**.
- Text or **single image-to-video**.

**Not this skill:** **cute healing** default—unless **cute horror** hybrid explicit.

**Models:** tiered—**Sora 2 Pro** (default best), **Kling 3.0** (good), **Pika 2.2** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Sora 2 / 2 Pro**: **no** generated audio—use `generate_audio` **false** or omit; choose `duration` from **4 / 8 / 12**. **Pika 2.2**: **do not** send `aspect_ratio` (text or image per frozen row).
...
- **Look / materials / VFX (genre):**
  - **Palette:** low saturation, sick green or cold blue bias **optional**.
  - **Space:** long empty vertical corridor, door at top third.
  - **Scares:** imply presence; **avoid** explicit gore default—escalate only if user requests.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** slow exploration, sudden snap push **one beat**, subtle handheld unease.
  - **Framing:** **9:16**—use height for dread; keep escape direction clear if chase.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `bright comedy, cute sparkles default, watermark`.
...
- `9:16` haunted hall—flicker bulb, slow walk, sudden push, drone + buzz SFX
- Mirror beat—breath fog, off-sync reflection **subtle**
- Image-to-video: spooky girl still → eye shift + light die
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `SORA_2_PRO` | 8 | **Sora 2 Pro**—**no audio**; **8s** **hallway** realism. |
| 👍 Good | `KLING_V3_0_PRO` | 10 | **Kling 3.0**—**anime** horror with **whisper** audio. |
| ⚡ Fast | `PIKA_2_2` | 10 | **Pika 2.2**—graphic **flash** cuts; **silent**. |
...
| Field | Value |
|-------|--------|
| `model` | `SORA_2_PRO` (unless user picks another **listed** tier) |
