---
name: dark-fantasy-anime-video-gen
version: 1.0.0
description: "Render dark fantasy anime dread—crimson/indigo gloom, cursed motifs, gothic architecture, and oppressive light for vertical horror-fantasy. Use when you need cursed worlds, cathedral ruins, or brooding motion from dark key art; expand briefs before submit. Paid API; **KLING_V3_0_PRO** default **gothic** anime; **WAN_2_6** for **heavy neg** mood passes (**silent**); **KLING_V2_5 Turbo** for **fast** action without **aspect_ratio**."

tags: [anime, dark-fantasy, gothic, horror-fantasy, cursed, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Dark fantasy anime vertical video

**Dark fantasy anime** in one line: **blood-indigo palette**, **fog teeth**, **sigils** stained dark, **weight of curse**.

**Style anchor:** `dark fantasy anime, cursed world, gothic tone, ominous atmosphere, shadowy lighting`.

## Use when

- **Cathedral** nave, **throne** of thorns **generic**, **ritual circle** ember glow.
- Text or **single image-to-video**.

**Not this skill:** **bright idol stage** default.

**Models:** tiered—**Kling 3.0** (default best), **Wan 2.6** (good), **Kling 2.5 Turbo** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Kling 2.5 Turbo** (text): **do not** send `aspect_ratio` (metadata has none). **Wan 2.5 / 2.6**: **no** audio; **image** rows often **omit** `aspect_ratio`—see frozen table.
...
- **Look / materials / VFX (genre):**
  - **Light:** hard moon rim, underexposed mids, selective red accent; drifting ash/embers.
  - **World:** broken statues, chains **generic**, stained glass **abstract**.
  - **Mood:** slow menace; avoid explicit gore unless user requests and platform rules allow.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** creeping dolly, pull-back to reveal scale, shadow crawl along floor.
  - **Framing:** **9:16**—vertical pillars; keep face partial in shadow for dread.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `bright pastel idol, daylight comedy, watermark`.
...
- `9:16` cursed cathedral—slow dolly, red stained light, dark ambient drone
- Throne silhouette—fog crawl, chains glint
- Image-to-video: dark knight art → ember float + eye glint
...
| `WAN_2_6` | 5, 10, 15 | 9:16, 4:3, 1:1, 3:4, 16:9 | 720p, 1080p | No | Yes | 2000 |
| `KLING_V2_5_TURBO` | 5, 10 | *(do not send `aspect_ratio`)* | *(omit)* | No | Yes | 2000 |
...
| `WAN_2_6` | 5, 10, 15 | *(omit)* | 720p, 1080p | No | Yes | single `image`; **omit** `aspect_ratio` |
| `KLING_V2_5_TURBO` | 5, 10 | 9:16, 16:9, 1:1 | *(omit)* | No | Yes | single `image` (registry lists `aspect_ratio` for i2v—use **`9:16`** for vertical) |
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `KLING_V3_0_PRO` | 10 | **Kling 3.0**—rim **curse** light + **audio** drones. |
| 👍 Good | `WAN_2_6` | 10 | **Wan 2.6**—**negative_prompt** strong; **no** model audio. |
| ⚡ Fast | `KLING_V2_5_TURBO` | 10 | **Kling 2.5 Turbo**—**no** `aspect_ratio` param; **no** audio. |
...
> Skill: `dark-fantasy-anime-video-gen`.
