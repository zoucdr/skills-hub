---
name: anime-music-video-style-video-gen
version: 1.0.0
description: "Rhythmic anime MV-style montage—emotional highlights, stylized cuts, lyric-like visuals for vertical punch. Use when you need AMV energy **generic**, chorus-hit visuals, or motion from character art; expand briefs before submit. Paid API; **VEO_3_1** **8s** **chorus** punch; **KLING_V3_0_STA** **longer** **10/15s** **anime** builds; **VEO_3_1_FAST** **cheap** **8s** stings (image → **`CHATBOT_VEO_3_1_FAST`**)."

tags: [anime, music-video, montage, emotional, rhythmic, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Anime music video style vertical video

**Anime MV style** in one line: **beat-synced** pushes, **emotion flash**, **color grade shifts** per section **generic**.

**Style anchor:** `anime music video, emotional montage, rhythmic cuts, dramatic highlight moments, lyrical mood`.

## Use when

- **Chorus** hit pose, **rain** silhouette, **spark** transition **generic**.
- Text or **single image-to-video**.

**Not this skill:** **single static dialogue** shot default.

**Models:** tiered—**Veo 3.1** (default best), **Kling 3.0** (good), **Veo 3.1 Fast** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Veo 3.x** (text): `duration` **must be 8**; set `resolution` as allowed. **Image fast tier** uses **`CHATBOT_VEO_3_1_FAST`**, not `VEO_3_1_FAST`.
...
- **Look / materials / VFX (genre):**
  - **Edit feel:** describe **3–4** micro-beats within clip (intro lift, build, hit, resolve) without requiring true multi-clip API—use **single continuous** motion that **feels** cutty.
  - **Color:** shift warm→cool across beats **optional**.
  - **FX:** lens flare punch, particle burst on hit, speed ramp **verbal**.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** snap push, pull out reveal, orbit crescendo, brief dutch **optional**.
  - **Framing:** **9:16**—center punch moments.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `slow documentary, flat lighting, watermark`.
...
- `9:16` MV build—snap pushes, chorus hit flare, J-rock instrumental (generic)
- Rain silhouette spin—dramatic highlight moment
- Image-to-video: character still → wind + light strobe **subtle**
...
| `VEO_3_1_FAST` | **8 only** | 9:16, 16:9 | 720p, 1080p, 4k | Yes | No | 2000 |
...
| `CHATBOT_VEO_3_1_FAST` | **8 only** | 9:16, 16:9 | 720p, 1080p, 4k | Yes | No | single `image` |
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `VEO_3_1` | 8 | **Veo 3.1**—**max** **polish** per **8s** hit. |
| 👍 Good | `KLING_V3_0_STA` | 10 | **Kling Standard**—**montage** length flexibility. |
| ⚡ Fast | `VEO_3_1_FAST` | 8 | **Veo 3.1 Fast**—iterate **fast**; still **8s** cap. |
...
> Skill: `anime-music-video-style-video-gen`.
