---
name: mecha-anime-video-gen
version: 1.0.0
description: "Generate mecha anime shots—armored plates, reactor glow, impact sparks, and heavy industrial motion for vertical clips. Use when you need giant robot anime, hangar reveals, or animating mecha line art / key visual; expand briefs before submit. Paid API; **KLING_V3_0_PRO** for **armor** detail + **audio**; **SORA_2_PRO** for **weighty** realism comps (**silent**); **DOUBAO_1_PRO_FAST** for rapid **sketches**."

tags: [anime, mecha, robot, sci-fi, action, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Mecha anime vertical video

**Mecha anime** in one line: **tonnage**, **hydraulic weight**, **metal stress**, **core bloom**, **sparks** on impact.

**Style anchor:** `mecha anime, giant robot, metallic armor, mechanical details, energy core, cinematic impact`.

## Use when

- **Cockpit** HUD glance, **sword clash** of machines, **transformation** panels (generic).
- Text or **single image-to-video**.

**Not this skill:** **pure magical girl ribbon** default (can hybrid if user asks).

**Models:** tiered—**Kling 3.0** (default best), **Sora 2 Pro** (good), **Seedance 1.0 Pro Fast** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Sora 2 / 2 Pro**: **no** generated audio—use `generate_audio` **false** or omit; choose `duration` from **4 / 8 / 12**. **Seedance 1.x (`DOUBAO*`)**: set `resolution`; check `negative_prompt` column per row.
...
- **Look / materials / VFX (genre):**
  - **Materials:** brushed metal, panel seams, warning decals **generic**, heat discoloration near vents.
  - **VFX:** thruster bloom, shell casings **stylized**, shockwave rings.
  - **Scale:** foreground debris sells size.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** low-angle underfoot stomp, orbit around torso, snap-zoom to eye sensor.
  - **Framing:** **9:16**—vertical mecha poses; watch limb crop at edges.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `toy plastic look, soft romance pastel, watermark`.
...
- `9:16` mecha stomp—low angle, sparks, reactor flare, epic percussion (generic)
- Hangar lights on—slow push across armor panels
- Image-to-video: mecha key art → joint steam + eye light-up
...
| `DOUBAO_1_PRO_FAST` | 5, 10 | 9:16, 16:9, 4:3, 3:4, 1:1, 21:9 | 480p, 720p, 1080p | No | Yes | single `image` |
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `KLING_V3_0_PRO` | 10 | **Kling 3.0**—sparks, **metal** read, **impact** audio. |
| 👍 Good | `SORA_2_PRO` | 8 | **Sora 2 Pro**—**no audio**; use **8s** or **12s** for **mass** studies. |
| ⚡ Fast | `DOUBAO_1_PRO_FAST` | 10 | **Seedance 1.0 Pro Fast**—cheap **5/10s**; **no audio**. |
