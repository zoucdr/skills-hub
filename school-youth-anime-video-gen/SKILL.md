---
name: school-youth-anime-video-gen
version: 1.0.0
description: "Create bright school-life anime energy—uniforms, sunlit corridors, track fields, and friendship beats for vertical clips. Use when you need campus nostalgia, club fair vibes, or animating school character art; expand briefs before submit. Paid API; **SEEDANCE_2_0** for **sunflare** runs; **VIDU_Q3** for **club-fair** color; **DREAMINA_3** silent **5/10s** stylized youth (**pick `resolution`**)."

tags: [anime, school, youth, campus, friendship, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# School youth anime vertical video

**School youth anime** in one line: **lens flare summer**, **chalk dust light**, **running shoelaces**, **laugh mid-stride**.

**Style anchor:** `school anime, youthful energy, bright sunlight, friendship, campus nostalgia`.

## Use when

- **Roof lunch**, **club room** golden hour, **sports** trial **generic**.
- Text or **single image-to-video**.

**Not this skill:** **grim workplace drama** default.

**Models:** tiered—**Seedance 2.0** (default best), **Vidu Q3** (good), **Dreamina 3** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Seedance 2.0**: set `resolution` (`720p` default); **no** `negative_prompt`. **Vidu Q3**: pick `resolution` from frozen row. **Dreamina 3** (image): **omit** `aspect_ratio` per registry.
...
- **Look / materials / VFX (genre):**
  - **Light:** bright but kind; soft shadow under bangs; uniforms crisp **anime**.
  - **Props:** schoolbag, shoe locker, gym **generic**.
  - **Emotion:** earnest camaraderie; avoid bullying depiction unless user explicitly frames responsibly.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** run-along tracking, handheld-light hallway, upward tilt to sky from courtyard.
  - **Framing:** **9:16**—keep faces clear; background school architecture simplified.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `horror school ghost default, neon cyberpunk, watermark`.
...
- `9:16` track run—sun flare, friends laughing, handheld-light follow, pop-rock instrumental (generic)
- Roof lunch breeze—uniform ties move, gentle push-in
- Image-to-video: school portrait → hair + ribbon wind
...
| `DREAMINA_3` | 5, 10 | 9:16, 4:3, 3:4, 1:1, 16:9, 21:9 | 720p, 1080p | No | No | 2000 |
...
| `DREAMINA_3` | 5, 10 | *(omit)* | 720p, 1080p | No | No | single `image`; **omit** `aspect_ratio` |
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `SEEDANCE_2_0` | 10 | **Seedance 2.0**—**track** and **handheld** feel + **audio**. |
| 👍 Good | `VIDU_Q3` | 10 | **Vidu Q3**—bright **uniform** detail. |
| ⚡ Fast | `DREAMINA_3` | 10 | **Dreamina 3**—**no audio**; fast **poster** motion. |
...
> Skill: `school-youth-anime-video-gen`.
