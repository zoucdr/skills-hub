...
# Healing Ghibli-inspired anime nature video

**Healing Ghibli-inspired** in one line: **painterly greens**, **cloud shadows rolling**, **gentle adventure** without copying named films.

**Style anchor:** `ghibli inspired, healing nature, warm sunlight, hand painted feel, peaceful fantasy`.

## Use when

- **Hillside** road, **village** roofs, **train** far curve **generic**.
- Text or **single image-to-video**.

**Not this skill:** **explicit franchise shot recreation**—stay **inspired**, not **derivative** of specific titles; **no** studio logos.

**Models:** tiered—**Seedance 2.0** (default best), **Seedance 1.5 Pro** (good), **Veo 3.1** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Veo 3.x** (text): `duration` **must be 8**; set `resolution` as allowed. **Image fast tier** uses **`CHATBOT_VEO_3_1_FAST`**, not `VEO_3_1_FAST`. **Seedance 2.0**: set `resolution` (`720p` default); **no** `negative_prompt`. **Seedance 1.x (`DOUBAO*`)**: set `resolution`; check `negative_prompt` column per row.
...
- **Look / materials / VFX (genre):**
  - **Paint:** soft edges, watercolor sky feel **anime**, lush foliage layers.
  - **Light:** warm sun, dappled shade, reflective puddle.
  - **Tone:** sincere wonder; avoid copying iconic characters or scenes—use **original** subjects.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** slow lateral across meadow, gentle descend along river, follow bicycle **generic** silhouette.
  - **Framing:** **9:16**—vertical tree columns; horizon calm.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `photoreal, gritty cyberpunk, horror, watermark`.
...
- `9:16` hillside road—hand-painted anime clouds, warm sun, slow pan, woodwinds + piano (generic)
- River glint—dragonfly **generic**, peaceful fantasy
- Image-to-video: landscape still → grass wave + cloud shadow roll
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `SEEDANCE_2_0` | 10 | **Seedance 2.0**—**meadow** **loops**; **`720p`**. |
| 👍 Good | `DOUBAO_1_5_PRO` | 10 | **Seedance 1.5 Pro**—**audio** for **gentle** beds. |
| ⚡ Fast | `VEO_3_1` | 8 | **Veo 3.1**—**premium** **8s** **golden** hour (not a studio copy—**inspired** only). |
...
> Skill: `healing-ghibli-style-video-gen`.
