---
name: isekai-fantasy-anime-video-gen
version: 1.0.0
description: "Build isekai fantasy anime worlds—magic particles, grand castles, violet/blue glow, and portal-scale vertical reveals. Use when you need otherworld anime establishing shots, spell circles, or motion from fantasy key art; expand briefs before submit. Paid API; **KLING_V3_0_PRO** for **VFX-heavy** isekai; **VIDU_Q3** for painterly **magic**; **GROK_IMAGINE_VIDEO** for wide **world** drafts (**no** audio)."

tags: [anime, isekai, fantasy, magic, worldbuilding, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Isekai fantasy anime vertical video

**Isekai fantasy anime** in one line: **portal shimmer**, **floating runes**, **impossible architecture**, **hero dwarfed by scale**.

**Style anchor:** `isekai anime, fantasy world, magical particles, epic landscape, enchanted atmosphere`.

## Use when

- **Summon gate**, **dragon silhouette**, **floating islands**, **guild hall** glow.
- Text or **single image-to-video**.

**Not this skill:** **pure cyberpunk city** without fantasy hybrid request.

**Models:** tiered—**Kling 3.0** (default best), **Vidu Q3** (good), **Grok Imagine** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Vidu Q3**: pick `resolution` from frozen row. **Grok Imagine**: **no** audio; set `resolution` **480p** or **720p**.
...
- **Look / materials / VFX (genre):**
  - **Magic:** particles, sigils, mana ribbons—layered but readable; color bias blue/violet/gold.
  - **World:** wide fantasy reads even in vertical—foreground framing device (arch, branch, banner).
  - **Scale:** slow reveal of depth—stairs, bridges, sky layers.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** crane-up reveal, orbit around caster, dolly through gate threshold.
  - **Framing:** **9:16**—keep horizon curved lines level enough to avoid nausea unless stylistic.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `sci-fi only, office cubicle, photoreal, watermark`.
...
- `9:16` portal storm—runes spiral, pull back to hero silhouette, fantasy orchestra bed (generic)
- Floating castle approach—magical particles, slow crane
- Image-to-video: landscape painting → clouds move + sigil pulse
...
| `GROK_IMAGINE_VIDEO` | 5, 10, 15 | 9:16, 1:1, 16:9, 4:3, 3:2, 2:3, 3:4 | 480p, 720p | No | No | 2000 |
...
| `GROK_IMAGINE_IMAGE_TO_VIDEO` | 5, 10, 15 | 9:16, 1:1, 16:9, 4:3, 3:2, 2:3, 3:4 | 480p, 720p | No | No | single `image` |
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `KLING_V3_0_PRO` | 10 | **Kling 3.0 Pro**—sigils, particles, **negative_prompt** hygiene. |
| 👍 Good | `VIDU_Q3` | 10 | **Vidu Q3**—fantasy **scale** + **resolution** ladder. |
| ⚡ Fast | `GROK_IMAGINE_VIDEO` | 10 | **Grok**—quick **landmark** ideas; **silent**; **720p** typical. |
...
> Skill: `isekai-fantasy-anime-video-gen`.
