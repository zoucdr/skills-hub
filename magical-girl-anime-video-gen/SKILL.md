---
name: magical-girl-anime-video-gen
version: 1.0.0
description: "Create magical girl anime sequences—transformation glow, ribbons, star particles, and candy-color fantasy for vertical feeds. Use when you need transformation beats, wand sparkles, or cute-fantasy motion from illustration; expand briefs before submit. Paid API; **VEO_3_1** for **8s** sparkle **hero** moments; **SEEDANCE_2_0** for **ribbon** motion length; **KLING_V3_0_STA** for **neg**-clean **transformation** drafts."

tags: [anime, magical-girl, transformation, cute, fantasy, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Magical girl anime vertical video

**Magical girl anime** in one line: **pink/violet candy light**, **ribbon trails**, **star confetti**, **hero pose** sparkle pop.

**Style anchor:** `magical girl anime, transformation sequence, sparkles, ribbons, pastel magic, cute fantasy`.

## Use when

- **Transformation** staging, **wand twirl**, **team pose**, **sparkle shield**.
- Text or **single image-to-video**.

**Not this skill:** **grimdark horror** default—unless **corrupted magical girl** is explicit and ethically framed by user.

**Models:** tiered—**Veo 3.1** (default best), **Seedance 2.0** (good), **Kling 3.0** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Veo 3.x** (text): `duration` **must be 8**; set `resolution` as allowed. **Image fast tier** uses **`CHATBOT_VEO_3_1_FAST`**, not `VEO_3_1_FAST`. **Seedance 2.0**: set `resolution` (`720p` default); **no** `negative_prompt`.
...
- **Look / materials / VFX (genre):**
  - **FX:** ribbon physics **stylized**, gem glints, circular magic arrays, heart/ star motifs **generic**.
  - **Wardrobe:** frills, gloves, boots—keep silhouette readable in **9:16**.
  - **Tone:** earnest, bright; avoid sexualized default—keep **all-ages** magical girl unless adult user confirms tone.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** spin orbit, rise on light pillar, snap cuts on pose locks (describe count sparingly).
  - **Framing:** center subject; particles as foreground depth.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `gore, photoreal, grimdark, watermark`.
...
- `9:16` transformation beat—ribbons, star particles, rotating camera, cute pop instrumental (generic)
- Wand twirl—pastel magic ring expands
- Image-to-video: magical girl still → sparkle burst + hair float
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `VEO_3_1` | 8 | **Veo 3.1**—stable **stage** lighting + **audio** in **8s**. |
| 👍 Good | `SEEDANCE_2_0` | 10 | **Seedance 2.0**—longer **5–15s** twirls; **`720p`**. |
| ⚡ Fast | `KLING_V3_0_STA` | 10 | **Kling 3.0 Standard**—**negative_prompt** to suppress **horror** bleed. |
...
> Skill: `magical-girl-anime-video-gen`.
