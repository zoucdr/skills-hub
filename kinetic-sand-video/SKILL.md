...
4. After confirmation (and optional loop keywords), run `node scripts/video_gen.js wait --json '…'` (text-to-video); parse stdout.
...
> **Note:** Prefer public **`https`** URLs so the API can fetch references; private or login-gated hosts may fail. If the assembled `scripts/video_gen.js` supports local file paths, review/verify the script and explicitly consent before local read-and-upload to WeryAI.
