...
# Sports anime vertical video

**Sports anime** in one line: **impact slow-mo**, **speed lines** on sprint, **ball spin** read, **team** fist stack.

**Style anchor:** `sports anime, intense motion, sweat highlights, competitive spirit, dramatic teamwork`.

## Use when

- **Last second** shot, **dive** save, **relay** pass **glow** **generic**.
- Text or **single image-to-video**.

**Not this skill:** **mecha cockpit** default.

**Models:** tiered—**Kling 3.0** (default best), **Sora 2** (good), **Wan 2.5** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Sora 2 / 2 Pro**: **no** generated audio—use `generate_audio` **false** or omit; choose `duration` from **4 / 8 / 12**. **Wan 2.5 / 2.6**: **no** audio; **image** rows often **omit** `aspect_ratio`—see frozen table.
...
- **Look / materials / VFX (genre):**
  - **Bodies:** exaggerated muscle tension **anime**, sweat glints, uniform motion wrinkles.
  - **Environment:** stadium lights, court reflections **stylized**.
  - **Story:** one clear play beat; avoid ambiguous ball physics—describe trajectory simply.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** tracking alongside sprint, low angle under jump, freeze emphasis on ball contact.
  - **Framing:** **9:16**—vertical jump reads; keep ball visible.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `slow romance walk default, watermark`.
...
- `9:16` buzzer shot—slow-mo jump, sweat glint, arena drums (generic)
- Track relay—speed lines, handoff spark stylized
- Image-to-video: athlete key art → wind + cloth motion
...
| `SORA_2` | 4, 8, 12 | 9:16, 16:9 | 720p | No | No | 2000 |
...
| `SORA_2` | 4, 8, 12 | 9:16, 16:9 | 720p | No | No | single `image` |
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `KLING_V3_0_PRO` | 10 | **Kling 3.0**—**speed lines** + **ball** read + **crowd** audio. |
| 👍 Good | `SORA_2` | 8 | **Sora 2**—**no audio**; **8s** **motion** study. |
| ⚡ Fast | `WAN_2_5` | 10 | **Wan 2.5**—cheap **silent** comps. |
