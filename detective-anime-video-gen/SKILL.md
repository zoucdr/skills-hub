---
name: detective-anime-video-gen
version: 1.0.0
description: "Moody detective anime investigation—clue inserts, venetian blind shadows, deduction tension for vertical mystery. Use when you need noir-anime hybrids, evidence table reveals, or motion from detective illustration; expand briefs before submit. Paid API; **KLING_V3_0_PRO** **noir** anime + **audio**; **VIDU_Q3** **rain** **window** detail; **DREAMINA_3_PRO** **silent** **mood** boards."

tags: [anime, detective, mystery, suspense, noir, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Detective mystery anime vertical video

**Detective anime** in one line: **high-contrast shadows**, **macro clue** insert, **rain on window** code strings **generic**.

**Style anchor:** `detective anime, mystery clues, suspense mood, dramatic deduction, shadow contrast`.

## Use when

- **Evidence board** push-in, **alley** tail, **office** lamp cone.
- Text or **single image-to-video**.

**Not this skill:** **bright idol concert** default.

**Models:** tiered—**Kling 3.0** (default best), **Vidu Q3** (good), **Dreamina 3 Pro** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Vidu Q3**: pick `resolution` from frozen row. **Dreamina 3 Pro** (image): **omit** `aspect_ratio` and `resolution` per registry.
...
- **Look / materials / VFX (genre):**
  - **Palette:** cool desat with warm lamp pockets.
  - **Props:** photos **generic**, red string **abstract**, magnifier, ashtray **optional** period-agnostic.
  - **Mood:** intellectual tension; avoid graphic violence unless user specifies responsibly.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** slow push on clue, peeping lateral past blinds, rack focus **described** between foreground object and silhouette.
  - **Framing:** **9:16**—vertical blinds as graphic lines.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `bright comedy pastel, explosive shonen default, watermark`.
...
- `9:16` detective office—blind shadows, slow push to photo board, rain + piano tension
- Alley tail—figure silhouette, shallow depth anime
- Image-to-video: detective still → lamp flicker + smoke
...
| `DREAMINA_3_PRO` | 5, 10 | 9:16, 4:3, 3:4, 1:1, 16:9, 21:9 | *(omit)* | No | No | 2000 |
...
| `DREAMINA_3_PRO` | 5, 10 | *(omit)* | *(omit)* | No | No | single `image`; **omit** `aspect_ratio` / `resolution` |
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `KLING_V3_0_PRO` | 10 | **Kling 3.0**—**blinds**, **neg** for **comedy** washout. |
| 👍 Good | `VIDU_Q3` | 10 | **Vidu Q3**—**macro** clue inserts. |
| ⚡ Fast | `DREAMINA_3_PRO` | 10 | **Dreamina 3 Pro**—**no audio**; image route **omits** `aspect_ratio`. |
...
> Skill: `detective-anime-video-gen`.
