---
name: cyberpunk-city-video-gen
version: 1.0.0
description: "Generate cyberpunk city videos with WeryAI—night rain, neon magenta-violet-cyan clash, wet-street reflections, haze, and high-tech dystopia mood for tech, street, and concept shorts. Use when you need tunneling dolly, fast lateral moves, low-angle advance, orbit around hero; dark electronic or industrial beds; expand short briefs before submit. Paid API; default KLING_V3_0_PRO, draft SEEDANCE_2_0."

tags: [cyberpunk, neon, rain, night, futuristic, dystopia, sci-fi, street, tech, synthwave, dark-electronic, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Cyberpunk city & neon dystopia video

**Cyberpunk city video** in one line: **perpetual night**, **dense fog or drizzle**, **violet–magenta–electric-blue** neon clash, **wet asphalt mirrors**, and **towering future-sprawl**—**high-tech unease**, not sunny suburbia. Camera favors **tunneling forward push**, **fast lateral track** past signage, **orbit around a silhouetted figure**, and **low-angle creep** under overpass tubes. Audio leans **dark synthwave pulse**, **industrial percussion**, **distant siren-adjacent pads** (all **generic**—no named tracks), plus **rain foley** and **dripping metal**.

**Style anchor tag (optional in user briefs):** `cyberpunk_city_video`.

## Use when

- Someone asks for **cyberpunk**, **neon city**, **rainy reflections**, **futuristic dystopia**, **purple blue glow**, **tech-noir street**, **concept sci-fi** promos.
- **Text-to-video** or **image-to-video** (single reference still) with **9:16**, **16:9**, or **1:1**.
- Users mention **blade-runner mood** (generic descriptor only), **smog**, **hologram-style glow** (no trademarked IP), **synth industrial**, or phrases like *cyberpunk city, neon lights, rainy reflections*.

**Not this skill:** **bright daytime** realism, **pastel cottagecore**, or **clean Apple-store minimal** as the default look—unless the user explicitly overrides and accepts leaving the default cyberpunk grammar.
...
3. Collect **aspect**: default **`9:16`** for vertical street / character hero; use **`16:9`** for widescreen city vistas; **`1:1`** when the user requests square concept posts.
...
5. Validate JSON fields against the frozen tables: **Kling** → **omit `resolution`**; **Seedance** → include **`resolution`** when using that model; **`negative_prompt`** only for Kling (optional but recommended with a short line that rejects daylight pastoral looks).
...
`video_gen.js` does **not** auto-expand. Before **`submit-*`** or **`wait`**, turn short input into a **full English production `prompt`** aligned with **cyberpunk city** grammar—**wet neon, haze, and forward momentum over daylight clarity**.
...
- **Look & grade:** **night-only** palette; **neon tubes** and **LED ribbons** in **magenta, violet, cyan, cold blue**; **volumetric haze**; **rain-slick** ground with **stretched reflections**; **high contrast** with **lifted neon cores** and **deep shadows**. Avoid **golden-hour warmth**, **pastel cottage**, and **flat corporate** lighting unless the user wants “daylit cyber” hybrid.
- **World read:** **dense skyline**, **stacked signage** (generic glyphs, no real brand logos), **wet alley**, **overpass cables**, **steam vents**—sell **future megacity** without copying specific fictional franchises.
- **Composition:** **leading lines** from roads and rails; **silhouette** hero with **rim neon**; **layered depth** with foreground rain blur.
- **Camera grammar (pick 1–2):** **tunneling dolly** or **Steadicam push** down the street; **fast lateral track** past a wall of signs; **low-angle advance** with **puddle reflections**; **orbit** around subject at **walking speed** with **rain streaks** in backlight.
- **Audio (mandatory when `generate_audio` is true`):** labeled **`Audio:`** block—**dark synthwave** arp, **industrial** kick and metal percussion, **sub pulse**, **rain and distant traffic** foley, **occasional electrical hum** (generic). **No** copyrighted music or recognizable branded cues. If the user wants **silent** video: set **`generate_audio`: `false`** and omit audio from the prompt; state that in the confirmation table.

**Style anchors (weave naturally):** *cyberpunk city, neon lights, rainy reflections, futuristic dystopia, purple blue glow, cyberpunk_city_video.*

**Length:** Stay within **`prompt_length_limit` 2000** for the selected model in the frozen table; trim redundant adjectives before losing rain, neon clash, and camera move.
...
- Do **not** request or depict **named copyrighted characters**, **real brand logos**, or **trademarked franchise** set-dressing; keep **generic** futuristic city language.

### Example prompts

- `9:16 rain-soaked alley—cyberpunk city, magenta cyan neon clash, low-angle puddle reflections, tunneling push, dark synthwave and industrial percussion`
- `16:9 skyline sprawl—volumetric haze, purple blue glow, fast lateral track past stacked signage, rain foley, sub pulse`
- `Orbit hero on wet street—rim neon, futuristic dystopia, cyberpunk_city_video, distant siren-adjacent pad (generic)`
- `Concept tech promo—neon ribbons, rainy reflections, Steadicam advance under overpass, electrical hum bed`
- `Animate this still—rain streaks, orbit move, neon reflections on jacket (image URL in JSON)`
...
|------|-----------|------------------|--------|
| ⭐ Best (default) | `KLING_V3_0_PRO` | 10 (use **15** for longer rain-soaked glides) | **Omit `resolution`.** Optional **`negative_prompt`** e.g. `daylight pastoral, sunny suburb, flat office lighting, pastel cottagecore, watermark, heavy CGI, recognizable branded logos` |
...
| `duration` | `10` (use **`5`** for a punchy tunnel beat; **`15`** on Kling/Seedance for longer rain sequences) |
| `resolution` | **Only for `SEEDANCE_2_0`:** **`720p`** (or **`480p`**)—**omit for Kling** |
| `generate_audio` | **`true`** unless the user requests silent |
| `negative_prompt` | **Kling only** (optional); keep lines compatible with **cyberpunk city**—omit for Seedance |

---

## Scenario: Text-to-video cyberpunk city

1. Capture **setting** (alley, bridge, rooftop), **weather** (drizzle vs downpour), and **neon bias** (magenta-heavy vs cyan-heavy) or accept a minimal brief.
...
## Scenario: Image-to-video cyberpunk city

**Before use:** Prefer public **`https://`** URLs. Local paths only with verified runtime support and **explicit consent**.

1. Plan motion (**push**, **lateral track**, **orbit**) that **adds** rain and **neon bounce** while respecting subject silhouette if requested.
...
> Skill: `cyberpunk-city-video-gen`.
