...
4. After confirmation, in the terminal (`{baseDir}` = skill root):
   ...
   ```sh
   node {baseDir}/scripts/video_gen.js wait --json '{"model":"(confirmed model)","prompt":"(full English prompt)","aspect_ratio":"9:16","duration":10,"generate_audio":false}'
   ```
...
3. After confirmation (`{baseDir}` = skill root):

   ```sh
   node {baseDir}/scripts/video_gen.js wait --json '{"model":"KLING_V3_0_PRO","prompt":"(full English prompt)","aspect_ratio":"9:16","duration":10,"generate_audio":false}'
   ```
