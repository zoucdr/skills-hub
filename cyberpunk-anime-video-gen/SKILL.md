---
name: cyberpunk-anime-video-gen
version: 1.0.0
description: "Craft cyberpunk anime city loops—neon rain, hologram UI, wet asphalt reflections, and fast lateral energy for vertical shorts. Use when you need futuristic dystopia anime, alley chases, or neon motion from concept art; expand briefs before submit. Paid API; **KLING_V3_0_PRO** for **neon** complexity; **KLING_V2_6_PRO** for punchy **10s** fights; **VEO_3_1_FAST** **8s** synth-city sizzle."

tags: [anime, cyberpunk, neon, sci-fi, city, vertical, text-to-video, image-to-video, video-gen]

metadata: { "openclaw": { "emoji": "🎬", "primaryEnv": "WERYAI_API_KEY", "paid": true, "network_required": true, "requires": { "env": ["WERYAI_API_KEY"], "bins": ["node"], "node": ">=18" } } }
user-invocable: true
---

# Cyberpunk anime vertical video

**Cyberpunk anime** in one line: **magenta/teal/cyan** wash, **rain sheets**, **glitch UI**, **crowded vertical stacks**.

**Style anchor:** `cyberpunk anime, neon city, hologram UI, futuristic dystopia, rainy reflections`.

## Use when

- **Alley sprint**, **rooftop drone**, **vendor stall** holo ads, **motorcycle** light trails (generic).
- Text or **single image-to-video**.

**Not this skill:** **historical samurai** period piece unless **neon-samurai** hybrid is explicit.

**Models:** tiered—**Kling 3.0** (default best), **Kling 2.6** (good), **Veo 3.1 Fast** (fast draft). See frozen table for JSON field rules.
...
**Filter:** **Kling 3.0** rows: **omit `resolution`**; optional `negative_prompt`. **Kling 2.6** (image): **omit** `aspect_ratio` and `resolution` unless API updates. **Veo 3.x** (text): `duration` **must be 8**; set `resolution` as allowed. **Image fast tier** uses **`CHATBOT_VEO_3_1_FAST`**, not `VEO_3_1_FAST`.
...
- **Look / materials / VFX (genre):**
  - **Neon:** practical-feel sources—signs, strips, vehicle underglow; wet ground doubles color.
  - **UI:** holographic panels, kanji-latin mix **generic** (no real brand logos).
  - **Weather:** rain, steam vents, heat shimmer.
- **Camera / motion / 9:16 framing (genre):**
  - **Moves:** lateral whip along street, low-angle forward push, drone-like glide (anime speed).
  - **Framing:** **9:16**—vertical city stacks; keep subject separation from busy background via light rim.
...
- **Negative prompt:** Optional **Kling** `negative_prompt`: `medieval village, daylight pastoral, watermark, subtitles`.
...
- `9:16` neon alley chase—rain, hologram UI flicker, synthwave bed (generic)
- Rooftop city stack—slow glide, cyan/magenta wash
- Image-to-video: cyberpunk still → rain + sign flicker
...
> Skill: `cyberpunk-anime-video-gen`.
